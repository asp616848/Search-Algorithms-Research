/*
* @file TrainerBot.cpp
* @brief Trainer Bot - A strong opponent for training
* @date 2025-11-29
*/
...
class TrainerBot: public OthelloPlayer
{
    public:
        TrainerBot( Turn turn );
...
TrainerBot::TrainerBot( Turn turn )
    : OthelloPlayer( turn ), timeLimitReached(false)
{
    initializeWeights();
    timeLimit = 1.95; // Slightly more time
...
void TrainerBot::initializeWeights()
{
    // Modified weights for TrainerBot - More aggressive on corners and edges
    int mid[8][8] = {
    ...
    // Early game: avoid X and C squares even more
    int early[8][8] = {
    ...
    // Late game: corners still matter but internal positions gain value
    int late[8][8] = {
...
uint64_t TrainerBot::hashBoard(const OthelloBoard& board)
{
    uint64_t hash = 0;
    for(int i = 0; i < 8; i++) {
        for(int j = 0; j < 8; j++) {
...
bool TrainerBot::isCorner(int x, int y) {
    return (x == 0 || x == 7) && (y == 0 || y == 7);
}

bool TrainerBot::isXSquare(int x, int y) {
    return ((x == 1 || x == 6) && (y == 1 || y == 6));
}

bool TrainerBot::isCSquare(int x, int y) {
    return ((x == 0 || x == 7) && (y == 1 || y == 6)) ||
           ((x == 1 || x == 6) && (y == 0 || y == 7));
}

bool TrainerBot::isEdge(int x, int y) {
    return (x == 0 || x == 7 || y == 0 || y == 7) && !isCorner(x, y);
}

bool TrainerBot::adjacentToCorner(int x, int y, const OthelloBoard& board) {
...
int TrainerBot::countPieces(const OthelloBoard& board, Turn player)
{
    return (player == BLACK) ? board.getBlackCount() : board.getRedCount();
}

int TrainerBot::getMobility(const OthelloBoard& board, Turn player)
{
    return board.getValidMoves(player).size();
}

int TrainerBot::getPotentialMobility(const OthelloBoard& board, Turn player)
{
...
int TrainerBot::getCornersCaptured(const OthelloBoard& board, Turn player)
{
    int corners = 0;
...
int TrainerBot::getFrontierDiscs(const OthelloBoard& board, Turn player)
{
...
int TrainerBot::getEdgeStability(const OthelloBoard& board, Turn player)
{
    int stability = 0;
...
int TrainerBot::getParityScore(const OthelloBoard& board, Turn player)
{
...
int TrainerBot::evaluateBoard(const OthelloBoard& board, Turn player)
{
    Turn opponent = other(player);
...
int TrainerBot::scoreMoveOrdering(const Move& move, const OthelloBoard& board, Turn player, int ply)
{
    int score = 0;
...
vector<Move> TrainerBot::orderMoves(const list<Move>& moves, const OthelloBoard& board, Turn player, int ply)
{
...
int TrainerBot::minimax(OthelloBoard& board, int depth, int alpha, int beta, bool maximizing, int ply)
{
    if(depth == 0 || timeUp()) {
...
Move TrainerBot::getBestMove(const OthelloBoard& board)
{
...
Move TrainerBot::play( const OthelloBoard& board )
{
